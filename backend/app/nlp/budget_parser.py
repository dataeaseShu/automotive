...
_BUDGET_TRIGGERS = re.compile(
    ...
    r"([零一二两三四五六七八九十百千万亿\d\.]+\s*[万千百亿]?)"
    r"|([零一二两三四五六七八九十百千万亿]+(?:\s*[万千百亿])?)\s*(?:元|块|rmb|人民币|预算)"
    r"|(\d[\d,\.]*)\s*(?:元|块|rmb)",
    re.IGNORECASE,
)


def parse_budget(text: str) -> Optional[SlotField]:
    """
    从文本中抽取总预算（元，整数）。
...
def parse_bid_strategy(text: str) -> Optional[SlotField]:
    """
    解析出价策略。
    - 命中"手动"或"出价X元" -> 手动出价
    - 否则返回 None（调用方默认智能出价）
